...
int YOLOv5::inference(const cv::Mat& bgr, std::vector<Object>& objects)
{
#define MAX_STRIDE 64
#define YOLOV5_V62_ENCRYPT 1
...
#if YOLOV5_V62_ENCRYPT
        ex.extract(yolov5s_6_2_param_id::BLOB_output, out);//BLOB_output
#else
...
void YOLOv5::nms_sorted_bboxes(const std::vector<Object>& faceobjects, std::vector<int>& picked, float nms_threshold, bool agnostic)
{
    picked.clear();
...
cv::Mat YOLOv5::draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
    ...
    // cv::imwrite("/storage/emulated/0/Pictures/bug1.jpg",image);
    // cv::waitKey(0);
    return image;
